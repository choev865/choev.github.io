...
let toggleButton = document.getElementsByClassName("toggle-menu-btn")[0];
let moreButtonsContainers = document.getElementsByClassName("button-wrapper");

// Add click event listener to the toggle button
toggleButton.addEventListener('click', function() {
  // Loop through the collection of more buttons containers and toggle their display property
  for (let i = 0; i < moreButtonsContainers.length; i++) {
    if (moreButtonsContainers[i].style.display === 'none') {
      moreButtonsContainers[i].style.display = 'block';
    } else {
      moreButtonsContainers[i].style.display = 'none';
    }
  }
});
